...
import com.udacoding.transactionpurchaseapp.databinding.FragmentQuotationBinding
...
class QuotationFragment : Fragment() {
    ...
    private fun observeData() {
        with(viewModel) {
            ...
            loading.observe(viewLifecycleOwner, Observer {
                loadingFragment(it, binding.progressBar)
            })
            ...
            error.observe(viewLifecycleOwner, Observer {
                showError(activity?.applicationContext, it)
            })

            empty_id_user.observe(viewLifecycleOwner, Observer {
                context?.let { it1 -> requiredToast(it1, it, getString(R.string.id_not_found)) }
            })
        }
    }
        ...
        binding.recyclerView.adapter = context?.let { it1 ->
            HistoryAdapter(it1, it?.data, object : HistoryAdapter.OnClickListener {
                override fun detail(item: DataItemHistory?) {
                    ...
                    val intent =
                        Intent(activity?.applicationContext, DetailHistoryActivity::class.java)
    ...
    private fun initView() {
        with(viewModel) {
            historyTransaction(session.id_user, "", QUOTATION)
        }
    }
