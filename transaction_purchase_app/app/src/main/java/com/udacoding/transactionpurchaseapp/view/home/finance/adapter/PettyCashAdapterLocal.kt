...
class PettyCashAdapterLocal(
    private val context: Context,
    private val data: List<EntityPettyCash?>?,
    private val itemClick: OnClickListener
) : RecyclerView.Adapter<PettyCashAdapterLocal.ViewHolder>() {
    ...
    class ViewHolder(
        ...
        fun bind(item: EntityPettyCash?) {
            binding.textViewPurchase.text = toRinggit(item?.petty_cash?.toDouble() ?: 0.0)
            binding.textViewNote.text = item?.note
            binding.textViewDate.text = formatDate(item?.created_at)

            Glide.with(context)
                .load(Constant.Companion.url.BASE_URL_IMAGE_PETTYCASH + item?.image)
            ...
            binding.imageViewPurchase.setOnClickListener {
                itemClick.viewImage(Constant.Companion.url.BASE_URL_IMAGE_PETTYCASH + item?.image)
            }
        }
    }
