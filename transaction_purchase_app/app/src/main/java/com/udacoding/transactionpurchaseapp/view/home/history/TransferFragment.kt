...
import com.udacoding.transactionpurchaseapp.databinding.FragmentTransferBinding
import com.udacoding.transactionpurchaseapp.databinding.FragmentUnpaidBinding
...
class TransferFragment : Fragment() {
    ...
    private fun observeData() {
        with(viewModel){
            ...
            empty_id_user.observe(
                viewLifecycleOwner,
                Observer { context?.let { it1 -> requiredToast(it1, it, getString(R.string.id_not_found)) } })

        }
    }

    private fun showData(it: ResponseTransactionByUser?) {
        binding.recyclerView.adapter = context?.let { it1 -> HistoryAdapter(it1, it?.data, object : HistoryAdapter.OnClickListener {
    ...
    private fun initView() {
        with(viewModel){
            historyTransaction(session.id_user, TRANSFER, COMPLETED)
        }
    }
